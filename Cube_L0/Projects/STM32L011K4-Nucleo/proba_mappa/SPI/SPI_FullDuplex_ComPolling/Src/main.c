...
//			HAL_Delay(250);
		...
		if (i>115)i=0;
			nixie2(++i,i,i);
			HAL_SPI_TransmitReceive_IT(&SpiHandle, (uint8_t*) &nixie_bytes[1],(uint8_t *)aRxBuffer, 2);
			cs_1();
			//HAL_Delay(5);
			HAL_SPI_TransmitReceive_IT(&SpiHandle, (uint8_t*) &nixie_bytes[0],(uint8_t *)aRxBuffer, 2);
			cs_2();
			//HAL_Delay(5);
//		}
//		  ir_signal = 0;
